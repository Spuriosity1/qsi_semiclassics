...
int main (int argc, const char** argv) {
    ...
    // Required parameters
    if (argc < 4){
        fprintf(stderr, "Usage: qsi_defect_static INFILE DEFECTFILE OUTPATH [overrides....]\n");
    ...
    // Saves spins an B configs at end of simulation
    bool save_state;
    ...
    p.declare_optional("save_state", &save_state, false);

    //read the params
    std::filesystem::path infile_path(argv[1]);
    p.from_file(infile_path);
    ...
    p.from_argv(argc, argv, /*start idx of argv*/4);
    ...
    // Load the g,g' values on all plaquettes.
    // Strictmode checks that all plaquettes have been specified.
    
    simulate.load_couplings(defectfile, true);
