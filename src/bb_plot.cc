/* Executable to draw BB correlators
 *
 * Usage: bb_plot "system size" "input filename" "output filename" [block]
...
int main(int argc, char** argv) {
    ...
    double* bb = new double[(4*n+1)*(6*n+1)];
    int i = 0;
    ...
    fwrite(bb, sizeof(double), (4*n+1)*(6*n+1), out);
    fclose(out);
